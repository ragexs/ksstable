class AvrsDatatable
  ...
  def fetch_avrs
    ...
    #Savrs = avrs.where(date_off: nil) unless params[:mdu].present?
    avrs = avrs.order("#{sort_column} #{sort_direction}")
    avrs = avrs.page(page).per_page(per_page)
    if params[:search][:value].present?
      avrs = avrs.joins(:mmm).where("mdu like :search or adress like :search or material like :search", search: "%#{params[:search][:value]}%")
    end
    avrs
  end
  ...
  def sort_column
    columns = %w[date_off mmms.mdu mmms.adress type_avr material comment users.email  date_on ]
